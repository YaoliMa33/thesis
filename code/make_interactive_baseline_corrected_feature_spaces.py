...
<div id="wrap">
  <header>
    ...
    <p>All current alcohol / acetone / air CSV files are included. Each point is an instantaneous sample after subtracting that file's first 60 s baseline.</p>
  </header>
  <div id="stage">
...
def build_feature_space(slug: str, sensors: list[str], label: str, record_map: dict[str, dict[str, object]], days: list[str]) -> tuple[Path, pd.DataFrame, dict[str, int]]:
    ...
    out_path = figures / f"interactive_bc_{slug}_feature_space.html"
...
def main() -> None:
    ...
    pd.concat(scales, ignore_index=True).to_csv(tables / "interactive_bc_feature_space_scales.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame(summaries).to_csv(tables / "interactive_bc_feature_space_summary.csv", index=False, encoding="utf-8-sig")
