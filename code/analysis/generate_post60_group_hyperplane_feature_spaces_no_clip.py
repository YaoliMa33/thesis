...
def build_feature_space(slug: str, sensors: list[str], label: str, record_map: dict[str, dict[str, object]], days: list[str]) -> tuple[Path, pd.DataFrame, pd.DataFrame, dict[str, object]]:
    ...
    out_path = figures / f"interactive_bc_no_clip_post60_group_hyperplane_{slug}.html"
...
def main() -> None:
    ...
    pd.DataFrame(summaries).to_csv(OUT_ROOT / "tables" / "alcohol_acetone_no_clip_post60_group_hyperplane_summary.csv", index=False, encoding="utf-8-sig")
    pd.concat(predictions, ignore_index=True).to_csv(OUT_ROOT / "tables" / "alcohol_acetone_no_clip_post60_group_hyperplane_predictions.csv", index=False, encoding="utf-8-sig")
    pd.concat(scales, ignore_index=True).to_csv(OUT_ROOT / "tables" / "interactive_bc_no_clip_post60_group_hyperplane_scales.csv", index=False, encoding="utf-8-sig")
